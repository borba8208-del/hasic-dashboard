...
# MAPA KATEGORIÍ: Klíč pro UI -> Název CSV souboru (bez přípony)
CATEGORY_MAP = {
    "HP": "HP",
    "Nahrady": "Nahrady",
    "Voda": "VODA",
...
# ==========================================
# 2. LOGIKA CENÍKŮ A NORMALIZACE
# ==========================================
def normalize_category(cat_key: str) -> str:
    """Převede klíč kategorie na název SQL tabulky dle zadání."""
...
def import_all_ceniky():
    """Hromadný import CSV souborů do deterministických SQL tabulek."""
    log = []
    conn = sqlite3.connect(DB_PATH)
    
    for ui_key, filename in CATEGORY_MAP.items():
        file_path = os.path.join(CSV_FOLDER, f"{filename}.csv")
        table_name = normalize_category(ui_key)
        
        if os.path.exists(file_path):
            try:
                # Čtení s českým oddělovačem ;
                df = pd.read_csv(file_path, sep=';', encoding='utf-8')
                df.columns = [c.strip().lower() for c in df.columns]
                
                if 'nazev' in df.columns and 'cena' in df.columns:
                    # Očištění tabulky a vložení nových dat
                    cols = [c for c in df.columns if c in ['nazev', 'cena', 'jednotka']]
                    df[cols].to_sql(table_name, conn, if_exists="replace", index=False)
                    log.append(f"✅ {table_name}: {len(df)} položek (zdroj {filename}.csv)")
                else:
                    log.append(f"❌ {filename}.csv: Chybí sloupce nazev/cena")
            except Exception as e:
                log.append(f"❌ {filename}.csv: Chyba {str(e)}")
        else:
            log.append(f"⚠️ {filename}.csv: Soubor nenalezen")
            
    conn.close()
    return "\n".join(log)

def get_price(cat_key: str, item_name: str) -> float:
    """Získá cenu položky ze správné tabulky dle centrální mapy."""
...
def get_company_from_ares(ico):
    """Opravený hluboký parser ARES API dle zadání."""
...
def repair_all_customers_with_ares():
    ...
    conn.close()
    return f"Opraveno {fixed} záznamů, přeskočeno {skipped}."

# ==========================================
# 4. PDF ENGINE (LOGIKA & LAYOUT)
# ==========================================
...
def create_report_pdf(zakaznik, categories, total_zaklad, sazba, doc_title, note_text=""):
    ...
    for cat_key, items in categories.items():
        ...
        pdf.cell(15, 6, "Ks", border=1, align='C'); pdf.cell(35, 6, "Cena/jedn.", border=1, align='R'); pdf.cell(40, 6, "Celkem", border=1, align='R'); pdf.ln()
...
# ==========================================
# 5. STREAMLIT UI (ABECEDNÍ A STABILNÍ)
# ==========================================
st.set_page_config(page_title="Urbánek Pro v6.3", layout="wide", page_icon="🛡️")
...
st.caption("v6.3 | Deterministický Import | Central Category Map | 100% Diakritika")

tabs = st.tabs(["🔥 Hasicí přístroje", "📦 Náhrady", "🚰 Požární vodovody", "🛠️ Ostatní", "🧾 Export"])

def item_row(cat_key, item_name, key, step=1.0):
    ...
    if not active: st.warning("Zadejte položky pro export.")
        ...
        if st.button("📄 VYGENEROVAT DOKUMENT PDF"):
            if not st.session_state.vybrany_zakaznik: st.error("Vyberte partnera v bočním panelu.")
                ...
                if pdf:
                    st.download_button("⬇️ STÁHNOUT PDF", data=pdf, file_name=f"Rozpis_{src_dl.replace('/','-')}.pdf")
