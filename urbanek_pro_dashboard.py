...
CATEGORY_MAP: Dict[str, str] = {
    ...
    "OZO": "OZO",
    "reklama": "reklama",
    "Servisni_ukony": "revize", # CSV revize se propisuje do bezpečné kategorie Servisni_ukony
}
...
def create_report_pdf(zakaznik: Dict[str, Any], items_flat: List[List[Any]], total_zaklad: float, sazba: float, doc_title: str, note_text: str = "") -> Optional[bytes]:
    ...
    pdf.set_text_color(200, 0, 0); pdf.set_font(pismo, "B", 12)
    ...
    if note_text:
        pdf.ln(10); pdf.set_font(pismo, "", 9); pdf.multi_cell(0, 5, note_text)
...
# 5. STREAMLIT UI 
# ==========================================
st.set_page_config(page_title="Urbánek Master Pro v9.0", layout="wide", page_icon="🛡️")
...
with st.sidebar:
    ...
    else: st.warning("Nelze načíst databázi partnerů.")
...
st.caption("Master Dashboard v9.0 | Ultimátní Stabilita | Boršov n. Vltavou")
...
with tabs[5]:
    ...
    if not active_items:
        ...
        firma = st.session_state.vybrany_zakaznik.get("FIRMA") if st.session_state.vybrany_zakaznik else "Neznámý"
        
        st.write(f"### Rozpis pro: {firma}")
        flat_list: List[List[Any]] = [[k, v["q"], v["p"]] for k, v in active_items.items()]
        table_rows = [{"Položka": row[0], "Ks": f"{row[1]:.2f}".rstrip("0").rstrip("."), "Celkem": f"{row[1] * row[2]:,.2f} Kč"} for row in flat_list]
        st.table(table_rows)
...
st.divider()
st.caption(f"© {datetime.date.today().year} {FIRMA_VLASTNI['název']} | Expert na požární ochranu od 1994 | Odborná certifikace TÜV NORD")    "Servisni_ukony": "revize",
}
