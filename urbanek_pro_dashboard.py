...
def safe_db_query(query: str, params: tuple = ()) -> pd.DataFrame:
    try:
        ...
        conn.close()
        # Garantujeme přítomnost sloupců pro UI i při prázdné DB
...
# =====================================================================
# ⚙️ LAYER 5: SERVICES (HLOUBKOVÝ REKURZIVNÍ SKENER)
# =====================================================================
...
def service_import_data():
    """Hloubkový importér: Prohledá celou Git strukturu v data/."""
    init_db()
    conn = sqlite3.connect(DB_PATH)
    logs = []
    ...
    if not all_files:
        return "⚠️ Nebyly nalezeny žádné soubory (.xlsx, .csv) v adresáři 'data/'."
    ...
    for file_path in all_files:
        ...
        # 1. IDENTIFIKACE ZÁKAZNÍKŮ (zakaznici, partneri, firmy)
        if any(x in fname for x in ["zakaznik", "partner", "firmy", "obchpartner"]):
            # Robot hledá firmu a ico inteligentně
        ...
        # 2. IDENTIFIKACE SKLADU (expimp, zbozi, sklad)
        ...
        # 3. IDENTIFIKACE CENÍKŮ (HP, VODA, NAHRADY)
        else:
...
# =====================================================================
# 🌐 LAYER 6: INITIALIZATION & UI (SIDEBAR)
# =====================================================================
init_db()
...
with st.sidebar:
    ...
    st.caption("Enterprise Terminal v55.0")
    st.divider()
    
    st.subheader("🏢 Výběr zákazníka")
    # Bezpečné získání partnerů
    ...
    if df_p.empty:
        ...
        if st.button("🔍 SPUSTIT HLOUBKOVOU SYNCHRONIZACI", type="primary", use_container_width=True):
            with st.spinner("Robot prohledává složky..."):
                res = service_import_data()
                st.success("Synchronizace dokončena.")
                st.code(res)
                st.rerun()
    else:
        # Pevná obrana proti KeyError při zobrazení
        options = ["-- Vyberte firmu --"]
        for _, r in df_p.iterrows():
            f = str(r.get('firma', 'Neznámý'))
            i = str(r.get('ico', ''))
            options.append(f"{f} | {i}")
        ...
        sel_p = st.selectbox("Pracovat pro:", options, index=curr_idx)
    ...
    if st.button("🗑️ Vyprázdnit košík", use_container_width=True):
...
if menu == "📝 Evidence & Zakázka":
    ...
    with tabs[0]:
        if not st.session_state.get("vybrany_zakaznik"):
            st.info("👈 Nejprve vyberte zákazníka v levém panelu (nebo spusťte synchronizaci).")
        else:
            vz = st.session_state["vybrany_zakaznik"]
            st.subheader(f"Evidence kontrol pro: {vz.get('firma')}")
            ...
            if st.button("💾 Uložit do databáze a přepočítat fakturu"):
                ...
                st.success("✅ Uloženo a faktura zaktualizována.")
                st.rerun()
...
# =====================================================================
    ...
    with t1:
        chosen = st.selectbox("Zobrazit tabulku:", ["obchpartner (Zákazníci)", "cenik_hp (Kontroly)", "cenik_zbozi (Sklad)"])
        actual_tbl = chosen.split(" ")[0]
        df_v = safe_db_query(f"SELECT * FROM {actual_tbl}")
        if df_v.empty: st.warning("⚠️ Tabulka je prázdná. Proveďte Synchronizaci.")
        st.dataframe(df_v, use_container_width=1)
        
    with t2:
        st.info("💡 Synchronizace prohledá hloubkově všechny složky v adresáři 'data' a spojí nalezené Excely.")
